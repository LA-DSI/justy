...
function searchBar() {
  ...
  let ul = document.getElementById("todos-container");
  let li = ul.getElementsByClassName("todo-wrapper");
  let a;
  let i;
  let txtValue;
  ...
  ul = document.getElementById("todos-done-container");
  li = ul.getElementsByClassName("todo-wrapper");
  for (i = 0; i < li.length; i += 1) {
    a = li[i].getElementsByTagName("p")[0];
    txtValue = a.textContent || a.innerHTML;
    if (txtValue.toUpperCase().indexOf(filter) > -1) {
...
function search() {
  ...
  const li = ul.getElementsByClassName("todo-wrapper");
  let a;
  let i;
  let txtValue;
  let count;
...
function searchDone() {
  const input = document.getElementById("search-bar");
  const filter = input.value.toUpperCase();
  const ul = document.getElementById("todos-done-container");
  const li = ul.getElementsByClassName("todo-wrapper");
  let a;
  let i;
  let txtValue;
        ...
        return response.json().then(function (json) {
          ...
          for (const todo of todos) {
            ...
            todoMain.children[1].onclick = function () {
              if (
                document.querySelector(".todo-desc") &&
                !document.getElementById(`todo-desc-${todo.id}`)
              ) {
                document
                  .querySelectorAll(".todo-desc")
                  .forEach((e) => e.remove());
                document
                  .querySelectorAll(".todo-wrapper")
                  .forEach((e) => (e.style.height = "55px"));
              ...
              if (document.getElementById(`todo-desc-${todo.id}`)) {
                document
                  .getElementById(`todo-desc-${todo.id}`)
                  .classList.add("slideOutUpDesc");
                ...
                const date = new Date(todo.endDate).toLocaleDateString(
                  navigator.language,
            ...
            todoSettings.onclick = function () {
              document
                .getElementById(`settings-buttons-${todo.id}`)
                .classList.toggle("no-display");
              document
                .getElementById(`dots-icon-${todo.id}`)
                .classList.toggle("no-display");
              todoSettings.classList.toggle("settings-shown-circle");
              todoSettings.previousSibling.classList.toggle(
                "settings-shown-main"
              );
              if (window.settingsOpen) {
                closeProperties(window.settingsOpen);
...
function editTask(idTodo) {
  ...
  for (const todo of window.todos) {
    if (todo.id === idTodo) {
      ...
      const date1 = new Date(todo.endDate);
      const tzoffset = new Date().getTimezoneOffset() * 60000;
      const date = new Date(date1 - tzoffset).toISOString().slice(0, -1);
  ...
  document.getElementById("edit-button").onclick = async function () {
    const dateEdit = new Date(document.getElementById("date-edit").value);
    let categoryEdit;
...
async function reverseDone(item_id, donePar) {
  let done = donePar;
  done = !done;
  await fetch("https://justy-backend.herokuapp.com/todos/edit", {
    method: "post",
...
function dropDownDone() {
  document
    ...
    .classList.toggle("no-display");
  document
    .getElementById("stripe-icon-done-up1")
    .classList.toggle("no-display");
  document
    .getElementById("stripe-icon-done-up2")
    .classList.toggle("no-display");
  document
    .getElementById("stripe-icon-done-down1")
    .classList.toggle("no-display");
  document
    .getElementById("stripe-icon-done-down2")
    .classList.toggle("no-display");
}
